 ...
 *
 * Implements FIPS 180-4 Secure Hash Algorithm 256.
 */
 ...
 */
void driver_sha256_final(sha256_ctx_t *ctx, uint8_t digest[32])
{
    uint32_t i, idx;
    uint8_t bits[8];
    ...
    for (i = 0; i < 32; i++) {
        digest[i] = (ctx->state[i >> 2] >> ((3 - (i & 0x3)) * 8)) & 0xFF;
    }
}
 ...
 */
void driver_sha256(const uint8_t *data, uint32_t len, uint8_t digest[32])
{
    sha256_ctx_t ctx;
